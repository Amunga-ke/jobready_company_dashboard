...
export async function PATCH(
  ...
  try {
    ...
    if (score === undefined) {
      return NextResponse.json({ error: "score field is required" }, { status: 400 });
    }

    if (score !== null && (typeof score !== "number" || score < 1 || score > 10)) {
      return NextResponse.json({ error: "score must be null or a number between 1 and 10" }, { status: 400 });
    }
